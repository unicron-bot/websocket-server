    ...
    createRoute() {
        this.router.get('/:guild_id/:member_id', async (req, res) => {
            try {
                ...
                const member = await this.server.managers.member(guild_id, member_id).catch((e) => { throw e; });
        ...
        this.router.post('/:guild_id/:member_id', async (req, res) => {
            try {
                ...
                const member = await this.server.managers.member(guild_id, member_id).catch((e) => { throw e; });
                await member.update(partial).catch((e) => { throw e; });
                await member.fetch().catch((e) => { throw e; });
                this.server.ws.local.emit('raw', 'memberUpdate', member.toJSON());
                res.status(200).send(member.toJSON());
            } catch (e) {
                res.status(400).send(e);
        ...
        this.router.delete('/:guild_id/:member_id', async (req, res) => {
            try {
                ...
                const member_id = req.params.member_id;
                const member = await this.server.managers.member(guild_id, member_id).catch((e) => { throw e; });
                await member.destroy().catch((e) => { throw e });
